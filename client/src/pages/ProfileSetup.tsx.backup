...
const INTEREST_OPTIONS = [
  'BDSM', 'Role-playing', 'Swinging', 'Group sex', 'Voyeurism', 'Exhibitionism',
  'Fetishes', 'Kink exploration', 'Dom/Sub', 'Polyamory', 'Open relationships',
  'Threesomes', 'Casual encounters', 'NSA fun'
]

const PREFERENCE_OPTIONS = [
  'Soft swap', 'Full swap', 'Same room', 'Separate rooms', 'Threesomes',
  'Foursomes', 'Group parties', 'Club events', 'Private meetings', 'Online play',
  'Video chat', 'Photo exchange'
]

const BOUNDARY_OPTIONS = [
  'Anal sex', 'Oral sex', 'BDSM', 'Pain play', 'Humiliation', 'Public play',
  'Recording/photos', 'Substance use', 'Rough play', 'Age play', 'Pet play',
  'Water sports', 'Scat play'
]

const SAFE_SEX_OPTIONS = [
  'Condom use', 'Dental dam use', 'Regular STI testing', 'Recent test results',
  'Vaccination status', 'Birth control', 'PrEP usage', 'Open communication',
  'Safe words', 'Consent discussions'
]
  ...
  const onSubmit = async (data: ProfileSetupData) => {
    ...
    try {
      ...
      if (error?.message?.includes('authentication') || error?.message?.includes('password authentication failed')) {
        errorMessage = 'Database connection failed. The server is experiencing issues. Please try again in a few minutes.'
      ...
      <div className="grid grid-cols-2 gap-2">
        {options.map((option) => (
          <div key={option} className="flex items-center space-x-2">
            ...
            />
            <Label htmlFor={option} className="text-sm text-white">{option}</Label>
          </div>
        ))}
      </div>
      {onCustomChange && (
            ...
            <div className="space-y-6">
                      ...
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => {
                          ...
                          if (checked) {
                            ...
                            form.setValue('seekingGenders', current.filter(g => g !== gender.toLowerCase()))
                          }
                        }}
                      />
                      <Label htmlFor={`seeking-${gender.toLowerCase()}`} className="text-white">{gender}</Label>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 gap-2">
                  {['Single male', 'Single female', 'Couple (male/female)', 'Couple (male/male)', 'Couple (female/female)', 'Group'].map((type) => (
                      ...
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => {
                          ...
                          if (checked) {
                            ...
                            form.setValue('seekingAccountTypes', current.filter(t => t !== type.toLowerCase()))
                          }
                        }}
                      />
                      <Label htmlFor={`seeking-type-${type.toLowerCase().replace(/[^a-z0-9]/g, '-')}`} className="text-white">{type}</Label>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label htmlFor="ageRangeMin" className="text-sm text-white">Min Age</Label>
                    <Input
                      id="ageRangeMin"
                      type="number"
                      min="18"
                  ...
                  <div>
                    <Label htmlFor="ageRangeMax" className="text-sm text-white">Max Age</Label>
                    <Input
                      id="ageRangeMax"
                      type="number"
                      min="18"
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <p className="text-xs text-pink-400 text-center">
                Photos are optional - you can add them later from your profile settings.
              </p>
            </div>
          </div>
        )
          ...
          <div className="text-center mb-8">
                  ...
                  return (
                    <div
                      ...
                      className={`flex items-center justify-center w-10 h-10 rounded-full transition-colors flex-shrink-0 ${
                        ...
                        isCompleted ? 'bg-pink-500/20 text-pink-400' :
            ...
            <div className="mb-8">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-sm text-white/70">Step {currentStep} of {maxSteps}</span>
              </div>
